$(document).ready(function(){
    ...
    $('#myTable').on('click','.edu-button',function(){
        ...
        if (row.child.isShown()) {
            ...
            // Open this row to show education details
            var educationData = row.data().education;
            var educationHtml = '<table><tr><th>' + 'Degree' + '</th><th>' + 'School/College' + '</th><th>' + 'Start Date' + '</th><th>' + 'Passout Year' + '</th><th>' + 'Percentage' + '</th><th>' + 'Backlog' + '</th></tr>';

            for(let i=0;i<educationData.length;i++)
            {
                educationHtml += '<tr><td>' + educationData[i].degree + '</td><td>' + educationData[i].school + '</td><td>' + educationData[i].startDate + '</td><td>' + educationData[i].passoutYear + '</td><td>' + educationData[i].percentage + '</td><td>' + educationData[i].backlog + '</td></tr>';
            }
            educationHtml += '</table>';

            row.child(educationHtml).show();
            tr.addClass('shown');
            // Set colspan attribute of the parent td to 8
            tr.next('tr').children('td').attr('colspan', 8);   
            tr.next('tr').children('td').children('table').css({'width':'90%','margin':'auto','text-align':'center','margin-top':'2rem','margin-bottom':'2rem'});       
            tr.next('tr').css({'background-color':'#dbe3e3'})   ;   
        }
    ...
    var table = $('#myTable').DataTable({     
        ...
        columns:[
            ...
            {data: 'graduateYear' , className: 'dt-center'},
            {
                ...
                defaultContent: '<button class="edu-button btn btn-primary">More</button>',
                targets: -1,
                className: 'dt-center'
            },
            {
                ...
                data: null,
                defaultContent: '<button class="edit-button btn btn-primary"  data-bs-toggle="modal" data-bs-target="#exampleModal"><i class="fa-solid fa-pen-to-square" id="icon"></i></button>                      <button class="delete-button btn btn-red mt-2"><i class="fa-solid fa-trash"></i></button>',
                targets: -1,
                className: 'dt-center'
            }
        ]
